...
for c = 1:length(condition)
    for o = 1:length(oscillations)
        ...
        if freq(o) == 4
            cfg.eBOSC.F_fit = cfg.fBOSC.F(~(cfg.fBOSC.F >= 3 ...
                & cfg.fBOSC.F <= 6));
        elseif  freq(o) == 10
...
%             
%             figure; plot(log10(cfg.fBOSC.F),fit_to_use); hold on;
%             plot(log10(cfg.fBOSC.F),trial_aperiodic_mean)
%             
...
for c = 1:length(condition)
    ...
    cfg                         = [];
    cfg.freq                    = 4;
    cfg.amplitude               = 5; % SNR of the simulated oscillation
